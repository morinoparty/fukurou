 ...
 */
export function LogView({ run, log, compact = false }: LogViewProps) {
  ...
  return (
    <LoadedLog key={url} text={state.text} kind={log.kind} url={url} fileName={logFileName(run.id, log)} compact={compact} />
  );
}
...
function LoadedLog({ text, kind, url, fileName, compact }: LoadedLogProps) {
  ...
  const visible = useMemo(
    () => filterLines(parsed.lines, { query: deferredQuery, errorsOnly }),
    [parsed, deferredQuery, errorsOnly],
  );
  const filtering = deferredQuery.trim() !== "" || errorsOnly;
  ...
  return (
    <div>
      ...
      <LogLines
        // 絞り込み条件が変わったら表示範囲を先頭からに戻す
        key={`${deferredQuery}\u0000${errorsOnly}`}
        lines={visible}
