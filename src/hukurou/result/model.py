"""hukurou が1バージョン分の実行ごとに書き出す result.json の契約（schemaVersion 1）。

hukurou/ui はこのファイルだけを頼りに表示を組み立てる。互換性のルール:
...
class HukurouInfo(ContractModel):
    version: str
    portablemc: str
...
class LogInfo(ContractModel):
    # harness: hukurou 自身 / server: サーバーの latest.log / client: プレイヤーのクライアントの latest.log / crash: クラッシュレポート
